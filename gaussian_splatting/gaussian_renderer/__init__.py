...
def render(
    ...
    # print('means2D: ', means2D.shape) # [N,3]
    ...
    if colors_precomp is not None:
        gs_kp_score = pc.get_kp_score
        # features_precomp = torch.cat((colors_precomp, gs_descriptor, gs_marker), dim=1)
        features_precomp = torch.cat((colors_precomp, gs_kp_score), dim=1)
    ...
    if mask is not None:
        ...
        rendered_image, depth, opacity, radii = rasterizer(
            ...
            means2D=means2D,
            shs=shs,
            colors_precomp=features_precomp, # features_precomp colors_precomp
            opacities=opacity,
            scales=scales,
            rotations=rotations,
    ...
    # vis_filter = radii.cpu() > 0
    # print('vis_filter: ', vis_filter)

    # Those Gaussians that were frustum culled or had a radius of 0 were not visible.
    # They will be excluded from value updates used in the splitting criteria.
    return {
        "render": rendered_image[:3, :, :],
        "kp_prob": rendered_image[-1, :, :],
