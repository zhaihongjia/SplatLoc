...
from utils.utils import image_gradient, image_gradient_mask
...
class Camera(nn.Module):
    ...
    def init_from_dataset(dataset, idx, projection_matrix):
        ...
        if 'sp_kp_score' in ret.keys():
            kp_score = ret['sp_kp_score'].to(device=dataset.device)
        else:
            kp_score = None

        if 'encode_feat' in ret.keys():
            encode_feat = ret['encode_feat'].to(device=dataset.device)
        else: 
            encode_feat = None
    ...
    def init_from_gui(uid, T, FoVx, FoVy, fx, fy, cx, cy, H, W):
        ...
        return Camera(uid, None, None, T, projection_matrix, fx, fy, cx, cy, FoVx, FoVy, H, W, kp_score=None, encode_feat=None,)

    @staticmethod
    def init_from_pose(idx, dataset, pose, projection_matrix):
        return Camera(
            idx,
            None,
            None,
